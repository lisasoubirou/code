...
def make_optics(file_input,time_frac,n_k,option):
    ...
    #FODO cell
    # cell=[drift_qd]+ [drift_BSC, BSC, drift_BSC]*n_k+[drift_dd]+[
    #     drift_BNC, BNC, drift_BNC]*n_k+ [drift_dd] +[drift_BSC, BSC, drift_BSC]*n_k+[drift_qd]
    # cell_name=['drift_qd']+ ['drift_BSC', 'BSC', 'drift_BSC']*n_k+['drift_dd']+[
    #     'drift_BNC', 'BNC', 'drift_BNC']*n_k+ ['drift_dd'] +['drift_BSC', 'BSC', 'drift_BSC']*n_k+['drift_qd']
...
def plot_twiss_diff(tw_6d,tw_4d):
    plt.figure(figsize=(12, 12))
    plt.subplot(2, 1, 1)
...
if __name__ == "__main__":
    line_cell= make_optics(file_input,0,51,'var_k') #,line_ds,tw_4d,tw_6d 
    survey_cell=line_cell.survey(theta0=RCS.cell_angle/4)
    rec_cell=track(line_cell)
    line_cell_ref= make_optics(file_input,0,51,'var_ref') #,line_ds_ref,tw_4d,tw_6d
    survey_cell_ref=line_cell_ref.survey(theta0=RCS.cell_angle/4)
    rec_cell_ref=track(line_cell_ref)
    line_cell_mid= make_optics(file_input,0.5,51,'var_ref') #,line_ds_mid,tw_4d,tw_6d 
    survey_cell_mid=line_cell_mid.survey(theta0=RCS.cell_angle/4)
    ...
    plt.plot(survey_cell['s'], survey_cell['X']+rec_cell.x[0]-survey_cell_ref['X'], label='Inj')
